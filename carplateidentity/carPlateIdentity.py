...
def get_chars(car_plate):
    ...
    for i,addr in enumerate(char_addr_list):
        ...
        #cv2.imwrite('./carIdentityData/opencv_output/char.jpg', char_img)
...
if __name__ == '__main__':
    ...
    char_w,char_h = 20,20
    plate_model_path = os.path.join(cur_dir, './carIdentityData/model/plate_recongnize/model.ckpt-550.meta')
    char_model_path = os.path.join(cur_dir,'./carIdentityData/model/char_recongnize/model.ckpt-660.meta')

    img = cv2.imread('./carIdentityData/images/30.jpg')
    # 预处理
